...
//
// OSX-specific way of handling swyft: URIs and PaymentRequest mime types.
...
void PaymentServer::handleURIOrFile(const QString& s)
{
    ...
    if (s.startsWith("swyft://", Qt::CaseInsensitive))
    {
        Q_EMIT message(tr("URI handling"), tr("'swyft://' is not a valid URI. Use 'swyft:' instead."),
            CClientUIInterface::MSG_ERROR);
    }
    else if (s.startsWith(BITCOIN_IPC_PREFIX, Qt::CaseInsensitive)) // swyft: URI
    {
#if QT_VERSION < 0x050000
        QUrl uri(s);
#else
        ...
        else // normal URI
        {
            ...
            if (GUIUtil::parseBitcoinURI(s, &recipient))
            {
                ...
                Q_EMIT message(tr("URI handling"),
                    tr("URI cannot be parsed! This can be caused by an invalid Swyft address or malformed URI parameters."),
                    CClientUIInterface::ICON_WARNING);
