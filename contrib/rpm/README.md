...
The RPM spec file provided here is for Swyft-Qt 0.12.0 and builds on CentOS
...
reference the release tarball available from https://swyft.network/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Swyft git repository](https://github.com/swyft-project/swyft-core) but are not part of
...
Currently only `Source10` is used, for the example `swyft.conf` file.
...
in the swyft git repository. At present only an SVG version of the Swyft
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the swyft
...
needed when 0.12.1 is released, a proper fix is already in the Swyft git
master branch.
...
idea to build Swyft Core against a static version of that library compiled
...
The `swyft.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `swyft-server` package. At this point, most RPM based
...
The `swyft-server` package should have SELinux support. How to properly do
...
Swyft-Qt against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/swyft/). He has been
packaging Swyft for Fedora at least since 2012.
...
4. I renamed the `swyft` package that contains the Qt GUI to `bitcoin-core` as
that appears to be how the general population refers to it, in contrast to
`swyft-xt` or `swyft-classic`. I wanted to make sure the general population
