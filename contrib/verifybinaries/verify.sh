...
###   swyft.network and swyft.network and compares them.
...
WORKINGDIR="/tmp/swyft_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://swyft.network"
HOST2="https://swyft.network"
BASEDIR="/bin/"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'swyft-',
...
if [ $? -ne 0 ]; then
   echo "swyft.network failed to provide signature file, but swyft.network did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "swyft.network and swyft.network signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Swyft Core binary release signing key installed?"
   fi
