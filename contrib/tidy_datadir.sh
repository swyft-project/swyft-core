...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Swyft database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Swyft datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Swyft datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Swyft 0.7 datadir."
    ;;
  3)
    echo "Detected Swyft pre-0.8 datadir."
    ;;
  4)
    echo "Detected Swyft 0.8 datadir."
    ;;
esac
